...
import '../actions/domain/action_reminder.dart';
...
///
/// **Test and debug only.** It lives under `lib/` so the debug diagnostics
/// screen can seed a device with the same corpus the performance tests
/// measure — the only production reference to it sits behind `kDebugMode` in
/// the router, so a release build tree-shakes it. Nothing here contains a real name,
    ...
    final due = i % 5 == 4
        ...
        : ActionDue(
            dateOnly
                ...
                : DateTime(
                    base.year, base.month, base.day + offset, 9 + i % 12, 30),
          );
