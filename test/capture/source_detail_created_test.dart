...
SourceItem _textCapture() => SourceItem(
      ...
      pastedText: 'Your insurance policy INV-2048 is due for renewal. '
...
void main() {
  ...
  group('once an Action has been made from it', () {
    setUp(() async {
      await _repo.create(
        sampleAction('a1', title: 'Renew the insurance policy',
            sourceId: 'src-1'),
      );
    });
    ...
    detailTest('opening the Action is the primary control now',
        (tester) async {
      await pumpDetail(tester);
  ...
  group('when several were made from one capture', () {
    setUp(() async {
      await _repo.create(sampleAction('a1', title: 'Pay it', sourceId: 'src-1'));
      await _repo.create(
